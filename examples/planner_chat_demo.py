...
from farmos.planner import survey, recommend, price_summary, weather_summary
...
def main() -> None:
    ...
    try:
        weather = weather_summary("salem", recommended_date=rec.recommended_date)  # REAL (Open-Meteo)
    except Exception as e:                # offline: skip weather
        weather = None
    ...
    if weather:
        print(f"weather (REAL, {weather['source']}): avg {weather['avg_tmax_c']}/{weather['avg_tmin_c']}C, "
              f"{weather['total_rain_mm']}mm/{weather['rainy_days']}d, sowing date in horizon: "
              f"{weather['recommended_in_horizon']}")
    print()

    # ── LLM presents that data to the farmer ──
    print(f"── {MODEL} presenting to the farmer ──")
    print(present(OllamaClient(MODEL), QUESTION, data, price=price, weather=weather))
