...
| Drip detection | **Two complementary on-device detectors, both running during a drip run** — the trained Edge Impulse FOMO model finds the **emitters**; a classical OpenCV projection-profile detector finds the **tube** and produces the steering error. (Originally scoped as "model primary, classical fallback"; in practice they do different jobs and a drip run needs both.) This is the on-device "Physical AI" claim |
